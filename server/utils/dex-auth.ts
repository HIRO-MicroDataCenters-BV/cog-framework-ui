...
export async function getDexSessionCookie(
    ...
    try {
        ...
        if (response.status === 403 || response.status === 302 || response.status === 307) {
        ...
        if (loginResponse.status !== 302 && loginResponse.status !== 303 && loginResponse.status !== 307) {
            return {
                success: false,
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Unknown error during DEX authentication',
        };
    }
}
