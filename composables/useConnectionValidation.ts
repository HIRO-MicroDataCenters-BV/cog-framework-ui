...
export const useConnectionValidation = () => {
  ...
  const isValidConnection = (connection: Connection, nodes: AppNode[]) => {
    ...
    if (!sourceNode.data?.component || !targetNode.data?.component)
      return false;
    ...
    // UX rule: allow connecting any output to any input.
    // We only require that source/target nodes and the referenced handles exist.
    return Boolean(outputDef && inputDef);
  };
  ...
  const highlightCompatibleHandles = (
    ...
    if (params.handleType === 'source') {
      const h = (sourceNode.data?.component?.output_path || []).find(
        (o: ComponentPath) => o.name === params.handleId,
      );
      ...
      const h = (sourceNode.data?.component?.input_path || []).find(
        (i: ComponentPath) => i.name === params.handleId,
      );
      if (h) sourceType = h.type;
    }
    ...
    nodes.forEach((node) => {
      ...
      targetHandles.forEach((handle: ComponentPath) => {
        // UX rule: all opposite-side handles are considered compatible.
