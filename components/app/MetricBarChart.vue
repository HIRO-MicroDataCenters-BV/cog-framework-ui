<template>
  <div class="bg-card border rounded-lg overflow-hidden flex flex-col">
    <!-- Header -->
    <div class="flex items-center justify-between px-3 pt-2.5 pb-2 border-b border-border/50">
      <div class="flex items-center gap-1.5 min-w-0">
        <Icon name="lucide:grip-vertical" class="w-3.5 h-3.5 text-muted-foreground/40 flex-shrink-0" />
      ...
      <div class="flex items-center gap-0.5 flex-shrink-0 ml-2">
        <button class="p-0.5 rounded hover:bg-muted/50 transition-colors">
          <Icon name="lucide:maximize-2" class="w-3 h-3 text-muted-foreground/60" />
        </button>
        <button class="p-0.5 rounded hover:bg-muted/50 transition-colors">
          <Icon name="lucide:more-vertical" class="w-3.5 h-3.5 text-muted-foreground/60" />
        </button>
      </div>
    </div>
    ...
    <div class="px-3 pt-2 pb-1 flex-1">
      ...
      <div class="relative" style="height: 52px">
        ...
        <div
          ...
          :style="{ width: Math.min(barPct, 100) + '%', minWidth: numericValue > 0 ? '2px' : '0' }"
        >
        ...
        <!-- Axis line -->
        <div class="absolute left-0 right-0 border-t border-border/30" style="bottom: 16px" />
    ...
    <div v-if="runName" class="flex items-center gap-1.5 px-3 pb-2 pt-0.5">
      ...
      <span class="text-[10px] text-muted-foreground truncate">{{ runName }}</span>
    </div>
  </div>
</template>
...
const isNormalized = computed(() => numericValue.value >= 0 && numericValue.value <= 1)
...
const ticks = computed(() => {
  ...
  for (let v = 0; v <= max * 1.001; v += step) {
    ...
    result.push({
      value: rounded,
      label: Number.isInteger(rounded) ? rounded.toString() : rounded.toFixed(1),
      pct: (rounded / max) * 100,
    })
...
function getTickLabelStyle(pct: number, i: number): Record<string, string> {
  ...
  if (i === ticks.value.length - 1) return { left: '100%', transform: 'translateX(-100%)' }
