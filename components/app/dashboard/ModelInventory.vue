...
import type { ModelTypeStat } from '~/composables/useDashboard';
  ...
  <div class="rounded-xl border border-border/60 bg-card flex flex-col">
    <div class="flex items-center justify-between px-4 py-3 border-b border-border/50">
      <div class="flex items-center gap-2">
        <Icon name="lucide:bot" class="size-4 text-muted-foreground" />
        <span class="text-sm font-medium">Model Inventory</span>
      </div>
    ...
    <div class="flex-1 px-4 py-3 space-y-2 pb-4">
      ...
      <template v-else-if="stats.length === 0">
        <p class="text-xs text-muted-foreground py-4 text-center">No models registered</p>
      </template>
      <template v-else>
        <div
          v-for="stat in stats"
