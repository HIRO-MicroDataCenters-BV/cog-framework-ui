...
<template>
  <div class="rounded-xl border border-border/60 bg-card flex flex-col">
    ...
    <div v-if="loading" class="flex-1 flex items-center gap-5 px-4 py-4">
      <span class="size-[136px] shrink-0 rounded-full border-[16px] border-muted animate-pulse" />
    ...
    <!-- error -->
    <p v-else-if="error" class="flex-1 text-xs text-muted-foreground py-10 text-center">
      <Icon name="lucide:triangle-alert" class="size-4 inline mr-1 text-red-500" />
      Failed to load
    </p>

    <!-- empty -->
    <p v-else-if="stats.length === 0" class="flex-1 text-xs text-muted-foreground py-10 text-center">
      {{ emptyText }}
    </p>
    ...
    <div v-else class="flex-1 flex items-center gap-4 px-4 py-4">
      ...
      >
        <svg :width="SIZE" :height="SIZE" :viewBox="`0 0 ${SIZE} ${SIZE}`" class="-rotate-90">
          ...
          <circle
            ...
            :class="[seg.colorClass, hoveredKey && hoveredKey !== seg.key ? 'opacity-25' : 'opacity-100']"
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span class="text-2xl font-bold tracking-tight leading-none tabular-nums">{{ total }}</span>
          <span class="text-[10px] text-muted-foreground mt-0.5">{{ unit }}</span>
        </div>
        ...
        <div
          ...
          :style="{ left: `${tip.x}px`, top: `${tip.y}px`, transform: 'translate(-50%, -135%)' }"
        >
          <div class="flex items-center gap-1.5 text-xs font-medium">
            <span class="size-2 rounded-full bg-current" :class="hoveredStat.colorClass" />
            {{ hoveredStat.label }}
          </div>
      ...
      <!-- Legend -->
      <div class="flex-1 min-w-0 max-h-[176px] overflow-y-auto pr-1 -mr-1 space-y-0.5">
        <div
          v-for="stat in stats"
          :key="stat.label"
          class="flex items-center gap-2 rounded-md px-1.5 py-1 cursor-default transition-colors"
          :class="hoveredKey === stat.label ? 'bg-muted/60' : hoveredKey ? 'opacity-50' : ''"
          @mouseenter="hoveredKey = stat.label"
          @mouseleave="hoveredKey = null"
        >
          <span class="size-2.5 rounded-full bg-current shrink-0" :class="stat.colorClass" />
          <Icon :name="stat.icon" class="size-3.5 shrink-0" :class="stat.colorClass" />
          <span class="text-sm truncate flex-1">{{ stat.label }}</span>
          <span class="text-sm font-semibold tabular-nums">{{ stat.count }}</span>
          <span class="text-[10px] text-muted-foreground tabular-nums w-9 text-right">{{ share(stat.count) }}%</span>
        </div>
      </div>
    </div>
  </div>
