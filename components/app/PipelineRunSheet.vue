  ...
  >
    <div v-if="run" class="flex flex-col gap-4">
      ...
      >
        ...
        <span
          :class="[statusBadgeClass, 'text-[10px] font-medium px-2 py-0.5 rounded shrink-0']"
        >
          {{ currentStatus }}
        </span>
      </div>
      ...
      <div v-if="activeTab === 'io'" class="space-y-6">
        ...
        <section>
          ...
          <div v-if="outputParameters.length" class="divide-y border-t border-b">
            <div
        ...
        <section>
          ...
          <div v-if="outputArtifacts.length" class="divide-y border-t border-b">
            ...
            >
              ...
              <span class="text-xs text-muted-foreground text-right break-all font-mono">
                {{ artifact.path }}
              </span>
            </div>
          </div>
      ...
      <!-- Logs (placeholder) -->
      <div v-else class="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
...
const selectedTemplate = computed(() => {
  ...
  const templates: any[] = run.value?.pipeline_spec?.spec?.templates ?? [];
...
const selectedTask = computed(() => {
  if (!props.selectedNodeName || !run.value) return null;
  const tasks: any[] = run.value?.run_details?.task_details ?? [];
...
const dagTask = computed(() => {
  ...
  for (const tpl of templates) {
    ...
    const found = tpl.dag.tasks.find(
      (t: any) => t.template === props.selectedNodeName,
    );
...
const statusBadgeClass = computed(() => {
  ...
  if (state === 'succeeded') return 'bg-green-100 text-green-700 dark:bg-green-800 dark:text-green-100';
  if (state === 'failed') return 'bg-red-100 text-red-700 dark:bg-red-800 dark:text-red-100';
  if (state === 'running') return 'bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-100';
...
const resolveParamValue = (rawValue: string): string => {
  if (!rawValue) return '';
  const runtimeParams: Record<string, string> = run.value?.runtime_config?.parameters ?? {};
  const argsParams: any[] = run.value?.pipeline_spec?.spec?.arguments?.parameters ?? [];

  const argsMap = new Map<string, string>();
  argsParams.forEach((p) => { if (p?.name) argsMap.set(p.name, p.value ?? ''); });
...
const inputParameters = computed(() => {
  if (selectedTemplate.value) {
    const paramDefs: any[] = selectedTemplate.value.inputs?.parameters ?? [];
    const dagArgMap = new Map<string, string>();
    (dagTask.value?.arguments?.parameters ?? []).forEach((p: any) => {
      if (p?.name) dagArgMap.set(p.name, p.value ?? '');
    });

    return paramDefs.map((def: any) => ({
      name: def.name,
  ...
  argsParams.forEach((p: any) => { if (p?.name) map.set(p.name, p.value ?? ''); });
...
const inputArtifacts = computed(() => {
  ...
  return (selectedTemplate.value.inputs?.artifacts ?? []).map((a: any) => ({
    name: a.name,
    path: a.path ?? a.from ?? '—',
  }));
});
...
const outputParameters = computed(() => {
  if (selectedTemplate.value) {
    ...
    return (selectedTemplate.value.outputs?.parameters ?? []).map((p: any) => {
  ...
  templates.forEach((tpl: any) => {
    (tpl.outputs?.parameters ?? []).forEach((p: any) => {
...
const outputArtifacts = computed(() => {
  if (selectedTemplate.value) {
    return (selectedTemplate.value.outputs?.artifacts ?? []).map((a: any) => ({
      name: a.name,
      path: a.path ?? '—',
    }));
  }
...
) as any as Ref<Array<{ update_time: string; state: string }>>;
</script>
