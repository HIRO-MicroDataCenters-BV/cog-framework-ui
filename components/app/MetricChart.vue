...
const color = (d: { key: string }) => chartConfig.value[d.key]?.color || 'hsl(var(--muted))';
</script>
...
<template>
  <div class="flex flex-col items-center gap-1">
    <div class="relative w-16 h-16">
      <ChartContainer :config="chartConfig" class="w-full h-full">
        <VisSingleContainer :data="data" class="w-full h-full">
          <VisDonut :value="value" :color="color" :arcWidth="8" :padAngle="0.02" />
        </VisSingleContainer>
      </ChartContainer>
    ...
    <span class="text-xs text-muted-foreground text-center max-w-16 leading-tight">{{ label }}</span>
  </div>
</template>
